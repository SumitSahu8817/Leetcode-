...
import api from './api'; // Custom axios instance use karenge standard networking ke liye
...
function App() {
  ...
  // 1. STRICT AUTO-LOGIN & SESSION VERIFICATION FACTOR (Fixes Random login bug)
  useEffect(() => {
  ...
  // Fetch problems and live stats when authenticated
  useEffect(() => {
    if (token) {
      api.get('/problems')
        .then(res => {
  ...
  const handleSignupSubmit = async (e) => {
    ...
    try {
      ...
      const errorResponse = err.response?.data?.error || "Signup Failed! Constraints not matched.";
    ...
    <div className={`h-screen ${bgMain} font-sans flex flex-col overflow-hidden text-xs transition-all duration-500`}>
        ...
        <div className="flex items-center space-x-3">
          {token && (
            <div className="flex items-center space-x-2">
              <button type="button" onClick={fetchLeaderboard} className="p-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/30 rounded-xl transition-all duration-300 flex items-center space-x-1"><Trophy size={14} /> <span className="font-bold">Leaderboard</span></button>
              <div onClick={() => setShowProfileStats(!showProfileStats)} className="flex items-center space-x-2 bg-gradient-to-r from-amber-500/10 to-orange-500/10 hover:brightness-125 px-3 py-1.5 rounded-xl border border-amber-500/30 cursor-pointer shadow-inner transition-all duration-300 transform hover:scale-105">
          ...
          {token && <button type="button" onClick={() => { localStorage.clear(); sessionStorage.clear(); dispatch(logout()); window.location.href = '/'; }} className="p-2 rounded-xl text-red-400 bg-red-500/10 border border-red-500/20 hover:bg-red-500/30 transition-all duration-300"><LogOut size={14} /></button>}
        </div>
      </nav>
          ...
          <div className={`w-full max-w-4xl rounded-3xl p-5 flex flex-col shadow-2xl border-2 ${bgCard}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1 overflow-y-auto pr-1">
              {safeProblemsList.map((prob, index) => (
                <div key={prob?._id || index} onClick={() => { if(prob) { dispatch(setCurrentProblem(prob)); setShowProblemsOverlay(false); } }} className={`group p-4 rounded-2xl border-2 cursor-pointer flex items-center justify-between transition-all ${safeProblem?._id === prob?._id ? 'bg-amber-500/10 border-amber-500/60 shadow-sm' : isDark ? 'bg-[#15151b] border-[#22222a] hover:border-amber-500/30' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
                  <div className="flex items-center space-x-3">
      ...
      {authView === 'judge' && (
        <div className="flex p-3 flex-1 overflow-hidden h-[calc(100vh-50px)] relative">
          <div style={{ width: `${leftWidth}%` }} className={`flex flex-col rounded-xl p-4 overflow-y-auto shadow-sm ${bgCard} border border-gray-800/40 transition-none`}>
          ...
          <div onMouseDown={startResize} className={`w-2 h-full cursor-col-resize flex flex-col justify-center items-center relative group z-30 transition-all ${isDragging ? 'bg-amber-500/30' : 'hover:bg-amber-500/20'}`}>
            <div className={`w-[2px] h-6 rounded-full transition-all ${isDragging ? 'bg-amber-400 h-10 shadow-lg' : 'bg-gray-700 group-hover:bg-amber-500 group-hover:h-8'}`}></div>
          </div>

          <div style={{ width: `${100 - leftWidth}%` }} className="flex flex-col space-y-3 h-full overflow-hidden transition-none">
            <div className={`flex-1 border rounded-xl overflow-hidden shadow-md flex flex-col ${bgCard} border-gray-800/40`}>
              <div className="px-3 py-2 flex items-center justify-between bg-black/20 border-b border-gray-800/40 text-[10px] font-black tracking-wider uppercase">
                <div className="flex items-center space-x-2">
                  <button type="button" onClick={() => setShowProblemsOverlay(true)} className="p-1 bg-amber-500 hover:bg-orange-500 text-black rounded shadow" title="Open Tasks Archive Grid"><AlignJustify size={11} /></button>
                  <Code size={12} className="text-amber-400" /> <span className="text-gray-300 font-extrabold">Sandbox Workspace Terminal</span>
                  <button type="button" onClick={handleResetCode} className="p-1 text-red-400 bg-red-500/10 border border-red-500/20 hover:bg-red-500/30 rounded-md transition-all duration-300 flex items-center space-x-1 active:scale-95 group" title="Reset Code"><RefreshCw size={11} className="group-hover:rotate-180 transition-transform duration-500" /> <span className="text-[9px] font-bold">Reset</span></button>
            ...
            <div className={`h-44 border rounded-xl overflow-hidden shadow-lg flex flex-col ${bgCard} border-gray-800/40`}>
              <div className="flex items-center justify-between px-2 bg-black/20 border-b border-gray-800/40">
                ...
                <div className="flex items-center space-x-2 pr-1">
                  <button type="button" onClick={(e) => handleRun(e)} disabled={status !== 'Idle'} className="px-2.5 py-1 bg-gray-500/10 hover:bg-gray-500/20 text-gray-400 border border-gray-500/30 font-black rounded text-[11px] flex items-center space-x-1 disabled:opacity-40">{status === 'Running' ? <RefreshCw size={11} className="animate-spin" /> : <Play size={11} />}<span>Run Code</span></button>
              ...
              <div className="flex-1 p-2 overflow-hidden text-[11px]">
                {activeTab === 'input' ? (
                  <textarea value={customInput} onChange={(e) => dispatch(setCustomInput(e.target.value))} className="w-full h-full p-2 bg-black/10 border border-gray-800/40 rounded-lg outline-none font-mono text-xs resize-none text-white focus:border-amber-500" placeholder="Feed process stream vector array elements..." />
                ) : (
